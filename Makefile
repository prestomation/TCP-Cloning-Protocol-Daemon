...
DAEMON_HDRS = tcpdaemon/TCPDaemon.h  tcpdaemon/packets/IPCPackets.h tcpdaemon/packets/TCPPackets.h
...
ftpc:	$(CLIENT_SRCS)  mkd
	$(CPP) $(CFLAGS) -o bin/$@ $(CLIENT_SRCS) $(LIBS)

ftps:	$(SERVER_SRCS) mkd
	$(CPP) $(CFLAGS) -o bin/$@ $(SERVER_SRCS)  $(LIBS)

tcpdaemon:	$(DAEMON_SRCS) $(DAEMON_HDRS) mkd 
