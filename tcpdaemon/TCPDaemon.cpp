...
void TCPDaemon::Start(sockaddr_in trollInfo)
{
	...
	while(1)
	{
		...
		pollfd listeningSockets[50]; 
		listeningSockets[0].fd = mLocalSock;
		listeningSockets[0].events = POLLIN;
		int idx=1;

		ListeningMap::iterator iter;
		
		for(iter = mListeningMap.begin(); iter!= mListeningMap.end(); iter++)
		{
			listeningSockets[idx].fd = iter->first;
			listeningSockets[idx].events = POLLIN;
            idx++;
		...
		for (int i = 1; i < mNumListeningSockets; i++)
		{
			...
			if(listeningSockets[i].revents & POLLIN)
			{
				cout << "This socket has stuff!" << endl;
				mListeningMap[listeningSockets[i].fd]->ReceiveData();
			}
		}
	}
	//clean up
...
//When all data has been sent remove listening socket, called by TCPConn
void TCPDaemon::removeListeningSocket(int sockfd, TCPConn* acceptingConn)
{

	//cout << "Removing" << sockfd << " from mListeningMap" << endl;
	mListeningMap.erase(sockfd);
	mNumListeningSockets--;
}
